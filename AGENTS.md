...
Only `human`-class identities may sign a protected-branch commit, and `sscsb`
refuses to emit an `ai`-class key into `allowed_signers` at all — so an AI
signature cannot be made verification-valid no matter how the policy file is
edited.
