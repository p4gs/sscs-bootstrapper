...
| `sscsb skill install [--dry-run] [--force] [--path <p>]` | Write the bundled agent skill to `.claude/skills/sscsb/SKILL.md`. Refuses — exit `2`, nothing written — when a file is already there and differs, unless `--force`. |
| `sscsb skill print` | The bundled skill to stdout, verbatim, nothing else. Pipe it into a digest or a diff. |
| `sscsb skill check [--path <p>] [--format text\|json]` | Compare the installed skill against the copy compiled into this binary: exit `0` identical, `1` differs or missing, `2` could not look. It detects an edit made to the installed file by another agent, a hook, or anything else on the machine. It **cannot** detect a tampered `sscsb` — the check, the bytes and the digest all ship in one artifact. Every result also carries `binary trust` (`binary.trust` in JSON: `user-writable` / `not-user-writable` / `unknown`) and the boolean `narrow_claim_holds`, measured at run time by walking the binary's whole resolution chain — every ancestor directory to the filesystem root and every symlink hop — and asking the kernel about each. **`unknown` is the default and `not-user-writable` is the exception**: the strong verdict needs the chain fully walked, every link both unwritable *and* unowned, and a qualifying platform. Only `not-user-writable` (which implies `narrow_claim_holds: true`) means nothing that could edit the skill could also have edited the check; read `unknown`, and `binary.chain_complete: false`, as `user-writable`. `--format json` carries the chain itself under `binary.probes` — one row per path walked, with `role`, `path` and two kernel answers, `writable` (may this user write it now) and `owned` (does this user own it, and so may `chmod` it into writability) — so an agent can name *which* link is open instead of trusting the verdict; **either answer `true` is an open door**, and a user-owned binary at mode `0555` is a proven takeover, not a theoretical one. `binary.resolved_path` is present when the executable path is a symlink. Two more fields carry caveats a JSON-only consumer would otherwise never see: `binary.chain_start` is `invocation-path` or `pre-resolved`, and where it is `pre-resolved` (Linux `/proc/self/exe`, which the kernel resolved before the process started, so an intermediate symlink cannot appear on the chain at all) `binary.strong_verdict_available` is `false` and `trust` is never `not-user-writable` however shut the chain looks; `binary.unchecked_mechanisms` lists what no verdict here checks even at full strength — POSIX ACLs, BSD file flags, mount options, container image layering, process capabilities — so `narrow_claim_holds: true` is the floor of what an attacker must beat, not a proof. For the binary itself, verify the release asset against its Sigstore identity: [docs/skill.md](docs/skill.md) — whose recipe runs verbatim against `v0.4.0` and later, the first tag whose assets include `SKILL.md`. |
