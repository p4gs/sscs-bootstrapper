...
- D-18 (2026-09-01): **Increment — the SSCSB Scorecard (owner-directed).** Owner: a repo's Scorecard listing on the SSCSB Directory needs an authed scan of repo/org settings, "an architecture just like OpenSSF Scorecard's", plus a public submit-a-repo queue whose unauthenticated scans yield deliberately-partial results that funnel maintainers toward installing the publish workflow (one-click Create-PR/Create-Issue from the Directory). Three components, mirroring scorecard / scorecard-action / scorecard-webapp: (1) `sscsb score` in this repo — `emit` folds every registered control's outcome into one result document (score over determinate PASS/FAIL only; DEGRADED charged to a `completeness` tier, `partial` vs `complete`, never to the score — unknown is not failure; un-bootstrapped repos scored against `Config::defaults()`, recorded as `config: registry-defaults`, so the Directory can score arbitrary public repos); `verify` is the consuming side — fail-closed (unsigned = FAIL, not a shrug), cosign bundle verification pinned to `https://github.com/<slug>/.github/workflows/sscsb-scorecard.yml@refs/heads/<default branch>` with the branch fetched LIVE via gh (a document naming its own trusted branch would let any-branch signatures nominate themselves), exit 1 = verdict / 2 = could-not-check kept strictly apart. (2) A new default-OFF phase-5 control `sscsb-scorecard` installing `templates/workflows/sscsb-scorecard.yml` — house style (Harden-Runner first, SHA-pinned, `persist-credentials: false`, env-indirected run bodies), self-contained: cosign-installer, then download+verify the sscsb release tarball (sha256 + cosign verify-blob against `release.yml@refs/tags/<ver>` — identity confirmed empirically from the v0.2.1 bundle's Fulcio cert SAN, and the sha256/`gh release download` recipe executed verbatim against the real v0.2.1 assets), then `score emit --sign`, then upload-artifact `sscsb-scorecard`. Off by default as a DISCLOSURE decision (the signed result names every failing control in public; Rekor is a public log), a different reason than the infrastructure controls and said so in the registry comment. Template pins `SSCSB_VERSION: v0.3.0` — the release that first ships `sscsb score` MUST be tagged v0.3.0 or the template installs a version that can't score. (3) The Directory and a marketplace Action live in dedicated repos (owner chose p4gs/sscsb-directory + p4gs/sscsb-scorecard-action); the Directory PULLS artifacts rather than accepting pushes (no standing infra, no cross-repo credentials; trust rides the signature, not the transport). Residual trust stated out loud in docs/sscsb-scorecard.md: v1 verification proves the canonical workflow signed the result but not that the workflow was unmodified — the owner-self-doctoring gap OpenSSF closes by fetching the workflow at the cert's commit SHA and actionlint-rule-checking it is explicitly the Directory's ingestion-hardening milestone, and until then the bar equals `gh attestation verify --signer-workflow` (the same bar release-attest.yml sets). Score doctrine: INFO is non-scoring (informational by definition), Disabled excluded from the denominator, no-determinate-outcomes → `null` score rather than 0.0.

### K — Increment: SSCSB Scorecard result document + publish pipeline (owner-directed 2026-09-01)

- [x] ISC-162: `src/score.rs` — `build_document` runs all 45 registry controls and emits the scorecard result document (documentType `https://github.com/p4gs/sscs-bootstrapper/scorecard-result/v1`, schemaVersion 1) with per-control outcomes, determinate-only 0–10 score, completeness tier, and `config: repo|registry-defaults` provenance (probe: unit tests assert every control appears exactly once, counts are internally consistent, and an un-bootstrapped repo scores against defaults rather than erroring)
- [x] ISC-163: scoring math is pure and honest — `score_from_counts` returns `None` on zero determinate outcomes (a score over nothing is not 0.0), DEGRADED/INFO/disabled never enter the numerator or denominator; `completeness_tier` goes `partial` the moment one enabled control went unevaluated, and `enabled == 0` cannot be `complete` (probe: pure unit tests)
- [x] ISC-164: `sscsb score emit [--output|--stdout] [--sign]` — always exit 0 on success (the document is the deliverable; gating stays `verify`'s job); `--sign` reuses `provenance::cosign_sign_blob` + the `.sigstore.json` bundle-path convention; `--stdout --sign` refused (nothing to sign) (probe: fake-cosign emit test asserts the bundle lands beside the output)
- [x] ISC-165: `sscsb score verify <file> --repo <slug>` fails closed in order — not-JSON / wrong documentType / unknown schema major / claimed-slug mismatch / missing bundle are each FAIL verdicts (exit 1); cosign-missing or no-identity-source are operational (exit 2), because "could not check" must never print as "checked and failed" (probe: table of fake-doc tests + hidden-gh test asserting Err not verdict)
- [x] ISC-166: verify pins the certificate identity to the canonical workflow on the LIVE default branch — `expected_identity(slug, branch)` + gh `repos/{slug}` `--jq .default_branch`; the fake-gh + arg-recording fake-cosign test asserts cosign received exactly `…/sscsb-scorecard.yml@refs/heads/<live branch>` and the GitHub OIDC issuer (probe: `verify_pins_identity_from_the_live_default_branch_and_passes`)
- [x] ISC-167: new phase-5 control `sscsb-scorecard`, default OFF (disclosure decision, documented in the registry comment), dispatching to `verify_template_control`; full add-a-control cascade honored: registry + dispatch arm, `templates/compliance/map.json` entry (SSDF PS.3.2 + CRA Art. 13), `workflows::ARTIFACTS`, integration.rs must-NOT-install assertion, tool_orchestration `ALL` list, phase-5/README/AGENTS/skill docs, README count 44→45 (probe: registry ∀-tests + template ∀-audit + full suite)
- [x] ISC-168: `templates/workflows/sscsb-scorecard.yml` — house-style publish workflow (Harden-Runner first at the pinned SHA, all steps SHA-pinned with version comments, `permissions: read-all` + per-job narrowing with per-grant comments, `persist-credentials: false`, values reach shell via `env:`), self-contained install path verified against the REAL v0.2.1 release: `gh release download` patterns, `sha256sum --check` against the shipped `.sha256` format, cosign verify-blob identity `release.yml@refs/tags/<ver>` confirmed from the actual bundle's Fulcio cert SAN (probe: `every_workflow_template_passes_own_audit` + the recipe executed verbatim in-session; full cosign chain not completable in-sandbox — TUF CDN blocked by proxy — documented in the PR)
- [x] ISC-169: `Config::defaults()` (config.rs) — synthesized from `default_config_toml(None)` so a scored-then-bootstrapped repo cannot change verdicts by writing the identical config to disk; the `path` is a marker (probe: `an_unbootstrapped_repo_is_scored_against_registry_defaults_and_says_so`)
- [x] ISC-170: docs — `docs/sscsb-scorecard.md` (three components, two tiers, trust chain incl. the stated residual owner-self-doctoring gap and the Directory's rule-check milestone, pull-not-push rationale, disclosure stance), phase-5 section + table row, README phase table + docs list, AGENTS.md command rows + docs list (probe: tests/agents_md.rs)
